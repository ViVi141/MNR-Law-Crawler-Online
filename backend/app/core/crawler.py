...
class PolicyCrawler:
    ...
    def crawl_single_policy(self, policy: Policy, callback: Optional[Callable] = None, retry_count: int = 0) -> bool:
        ...
        try:
            ...
            if retry_count < max_policy_retries:
                ...
                # 重试次数已用完，记录失败
                logger.error(f"政策爬取失败 (已重试 {retry_count} 次): {policy.title} - {error_msg}")
    ...
    def _save_json(self, policy: Policy):
        ...
        safe_id = original_id.replace('|', '_').replace('/', '_').replace('\\', '_').replace(':', '_')
        
        # 如果ID过长或包含不安全字符，使用hash
        if len(safe_id) > 100 or not all(c.isalnum() or c in ('_', '-', '.') for c in safe_id):
    ...
    def _download_attachments(
        ...
        for i, att in enumerate(target_files, 1):
            ...
            if len(target_files) == 1:
                ...
                if (policy_title_base and safe_name_base and 
                    (policy_title_base in safe_name_base or safe_name_base in policy_title_base or
                     len(set(policy_title_base) & set(safe_name_base)) / max(len(policy_title_base), len(safe_name_base), 1) > 0.6)):
