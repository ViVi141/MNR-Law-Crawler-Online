            ...
            try:
                ...
                if content_div:
                    ...
                    if custom_style:
                        ...
                        # 手动清理每行的首尾空白，但保留换行结构
                        lines = content.split('\n')
                        content = '\n'.join([line.strip() for line in lines if line.strip() or line == ''])
                    else:
                ...
                for i, label in enumerate(labels_1):
                    if i < len(values_1):
                        ...
                        if '发文字号' in label_clean or '文号' in label_clean:
                            ...
                            if 'publisher' not in metadata or not metadata.get('publisher'):
                           ...
                           '效力' not in label and '级别' not in label:
                            ...
                            if any(keyword in value for keyword in ['年', '月', '日']) or len(value) >= 8:
                                if 'pub_date' not in metadata or not metadata['pub_date']:
                                    metadata['pub_date'] = value
                        elif '发布机构' in label or ('机构' in label and '发布' in label):
    ...
    def _clean_content(self, content: str) -> str:
        ...
        for line in cleaned_lines:
            ...
            if line in metadata_labels:
                ...
                is_metadata = True
            elif len(line) < 20 and any(keyword in line for keyword in ['年', '月', '日']):
                # 只对明确的日期格式或包含多个元信息关键词的行判断为元信息
                metadata_keywords_count = sum(1 for keyword in ['部门', '规范性文件', '现行有效', '废止', '机构', '文号'] if keyword in line)
