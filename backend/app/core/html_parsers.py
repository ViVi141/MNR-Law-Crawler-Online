            ...
            for row in rows:
                try:
                    ...
                    for i in range(len(cells) - 1):
                        label = cells[i].get_text(strip=True).replace('\xa0', '').replace(' ', '')
                        value = cells[i + 1].get_text(strip=True) if i + 1 < len(cells) else ''
                        
                        # 查找标题和链接
                        if '标题' in label or '名称' in label:
                            # 标题可能在当前列或下一列
                            link = cells[i + 1].find('a', href=True) if i + 1 < len(cells) else None
                            if not link:
                        ...
                        # 查找发布日期（优先使用'生成日期'，如果没有则使用'发布日期'）
                            ...
                            if value and (any(keyword in value for keyword in ['年', '月', '日']) or len(value) >= 8):
                                pub_date = value
