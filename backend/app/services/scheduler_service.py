...
class SchedulerService:
        ...
        try:
            ...
            for task in enabled_tasks:
                try:
                    ...
                    logger.error(f"加载定时任务失败: {task.task_name} - {e}", exc_info=True)
        except Exception as e:
    ...
    def create_scheduled_task(
        ...
        if is_enabled and self.scheduler and self.scheduler.running:
            try:
                ...
                logger.error(f"添加定时任务到调度器失败: {task_name} - {e}", exc_info=True)
                    ...
                    if email_service.is_enabled(db) and email_service.to_addresses:
                        ...
                        try:
                            ...
                            loop.run_until_complete(email_service.send_task_completion_notification(
                                ...
                                policy_count=task_info.get("policy_count", 0) if isinstance(task_info, dict) else 0,
    ...
    def enable_scheduled_task(self, db: Session, scheduled_task_id: int) -> ScheduledTask:
        """启用定时任务"""
        scheduled_task = db.query(ScheduledTask).filter(
            ScheduledTask.id == scheduled_task_id
        ).first()
        ...
        if self.scheduler and self.scheduler.running:
            try:
                ...
                logger.error(f"启用定时任务失败: {scheduled_task.task_name} - {e}", exc_info=True)
    ...
    def disable_scheduled_task(self, db: Session, scheduled_task_id: int) -> ScheduledTask:
        """禁用定时任务"""
        scheduled_task = db.query(ScheduledTask).filter(
            ScheduledTask.id == scheduled_task_id
        ).first()
        ...
        if self.scheduler and self.scheduler.running:
            ...
            try:
                ...
                logger.warning(f"从调度器移除任务失败: {scheduled_task.task_name} - {e}")
    ...
    def get_task_runs(
        ...
        total = query.count()
        runs = query.order_by(ScheduledTaskRun.run_time.desc()).offset(skip).limit(limit).all()
