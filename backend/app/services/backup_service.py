...
class BackupService:
        ...
        try:
            ...
            try:
                ...
                if email_service.is_enabled() and email_service.to_addresses:
                    ...
                    try:
                        loop.run_until_complete(email_service.send_backup_notification(
                            backup_type=backup_type,
            ...
            try:
                ...
                if email_service.is_enabled() and email_service.to_addresses:
                    ...
                    try:
                        loop.run_until_complete(email_service.send_backup_notification(
                            backup_type=backup_type,
                            backup_path=str(backup_path) if backup_path.exists() else "N/A",
                            file_size="0",
                            status="failed",
                            error_message=str(e),
                            start_time=backup_record.start_time,
    ...
    def restore_backup(
        ...
        try:
            backup_record = db.query(BackupRecord).filter(
                BackupRecord.id == backup_record_id
            ).first()
            ...
            if backup_record.local_path and Path(backup_record.local_path).exists():
                ...
                try:
                    s3_service = S3Service()
                    if s3_service.is_enabled():
                        backup_path = s3_service.download_file_to_temp(backup_record.s3_key)
                except Exception as e:
    ...
    def cleanup_old_backups(
        ...
        try:
            # 获取所有已完成的备份，按创建时间排序
            all_backups = db.query(BackupRecord).filter(
