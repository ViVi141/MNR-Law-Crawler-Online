...
class StorageService:
    ...
    def save_policy_file(
        ...
        try:
            ...
            if self.cache_service.is_enabled():
                cache_path = self.cache_service.cache_policy_file(
                    policy_id, file_type, source_path
                )
                if cache_path:
    ...
    def get_policy_file_path(
        self, policy_id: int, file_type: str, task_id: Optional[int] = None
    ) -> Optional[str]:
        ...
        if self.storage_mode == "local":
            local_path = (
                self.local_dir / "policies" / file_dir / f"{policy_id}.{file_ext}"
            )
            if local_path.exists():
                # 更新缓存
                if self.cache_service.is_enabled():
                    self.cache_service.cache_policy_file(
                        policy_id, file_type, str(local_path)
                    )
    ...
    def cleanup_policy_files(
        self, policy_id: int, task_id: Optional[int] = None
    ) -> bool:
        ...
        if self.storage_mode == "local":
            ...
            if task_id:
                ...
                attachments_dir = (
                    self.local_dir / "policies" / str(policy_id) / "attachments"
                )
    ...
    def save_attachment(
        ...
        """保存附件"""
        result = {"local_path": None, "s3_key": None, "success": False}
    ...
    def get_attachment_file_path(
        ...
        # 1. 检查缓存
        cache_path = self.cache_service.get_attachment_file(
            policy_id, attachment_filename
        )
        if cache_path:
            return str(cache_path)
        ...
        if self.storage_mode == "local":
            ...
            if local_path.exists():
                # 更新缓存
                if self.cache_service.is_enabled():
                    self.cache_service.cache_attachment_file(
                        policy_id, attachment_filename, str(local_path)
                    )
                return str(local_path)
        ...
        if self.s3_service.is_enabled():
            ...
            # 下载到缓存
            if self.cache_service.is_enabled():
                cache_path = self.cache_service.get_attachment_cache_path(
                    policy_id, attachment_filename
                )
                cache_path.parent.mkdir(parents=True, exist_ok=True)
    ...
    def list_attachments(
        ...
        if self.storage_mode == "local":
            ...
            if attachments_dir.exists():
                attachments.extend(
                    [f.name for f in attachments_dir.iterdir() if f.is_file()]
                )
    ...
    def cleanup_old_files(self, days: int = 30) -> dict:
        """清理旧文件"""
        result = {"cleaned_local": 0, "cleaned_s3": 0, "cleaned_cache": 0}
                            ...
                            try:
                                ...
                                for file_path in policy_dir.rglob("*"):
                                    if file_path.is_file():
                                        ...
                                        days_old = (current_time - file_mtime) / (
                                            24 * 3600
                                        )
                                        if days_old > days:
                                            file_path.unlink()
