...
class SystemConfig(Base):
    ...
    )  # feature/s3/email/scheduler/backup/cache/storage
...
class BackupRecord(Base):
    ...
    backup_strategy = Column(
        String(50)
    )  # always/on_success/on_new_policies/daily/weekly/monthly - 备份策略
    source_deleted = Column(Boolean, default=False)  # 来源是否已删除
    source_name = Column(
        String(255)
    )  # 备份时保存的任务名称（用于追溯，即使任务删除也能知道来源）
