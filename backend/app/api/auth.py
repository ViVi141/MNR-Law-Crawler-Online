...
def reset_password(
    ...
    """重置当前用户密码（不需要旧密码）"""
    try:
        success = AuthService.reset_password(db, current_user, password_reset.new_password)
        if not success:
            raise HTTPException(
                status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
                detail="密码重置失败"
...
""", "html", "utf-8"))
                ...
                try:
                    ...
                    return PasswordResetResponse(
                        success=True,
                        message=f"新密码已发送到 {to_email}，请查看邮件"
                    )
                except Exception as e:
